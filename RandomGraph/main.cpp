...
const int MAXN = 1e6;
string _filename =
    "D:/git/git/test/data/random_graph/general_random/group_1/"
    "n1500_t30_p3_b002_v07/"
    "animal";

const int NodesNum = 1500;          // total nodes number
const int pSize = 3;               // total partition number
const int tSize = 30;              // terminal number in each partition
double subGnodeRatio = 0.7;  // V_k/V
double subGarcRatio = 0.02;   // arc/maximum
const int GraphNum = 20;          // graph number generate
const int TreeSpanNodesNum = 5;
bool random_span = 1;
...
int main() {
    ...
    for (int GraphId = 1; GraphId <= GraphNum; GraphId++) {
        ...
        for (int k = 1; k <= pSize; k++) {
			...
			subGarcnum = (int)(1.0*subGnodenum * (subGnodenum - 1) / 2 * subGarcRatio);
